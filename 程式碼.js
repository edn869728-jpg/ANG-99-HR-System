...
function createUploadSession(payload) {
  ...
  const webAppUrl = getScriptProp_('WEB_APP_URL', DEFAULT_WEB_APP_URL) || ScriptApp.getService().getUrl();
  const uploadUrl = webAppUrl + '?page=upload&upload_id=' + encodeURIComponent(uploadId) + '&token=' + encodeURIComponent(token);
...
function buildAdminLoginResponse_(companyId, employee, token) {
  return ok_({
    ...
    role: employee.role || '',
    session_token: token,
    next_url: DEFAULT_FRONTEND_URL + 'admin.html?company_id=' + encodeURIComponent(companyId) + '&id=' + encodeURIComponent(normalizeUpper_(employee.employee_id || '')) + '&session_token=' + encodeURIComponent(token)
  });
}
...
// 給前端 GitHub / WebApp fallback 用，未設定 webAppUrl 時可暫用：
...
function apiVerifyAdminEmailCodeAndLoginV28_(payload) {
  ...
  return ok_({
    ...
    verify_token: token,
    session_token: token,
    next_url: DEFAULT_FRONTEND_URL + 'admin.html?company_id=' + encodeURIComponent(companyId) + '&id=' + encodeURIComponent(email || 'verified_admin') + '&role=Admin&verified=1&auth_email=' + encodeURIComponent(email) + '&token=' + encodeURIComponent(token)
  });
}
...
function apiAdminLoginByVerifiedAuthV28_(payload) {
  ...
  return ok_({
    ...
    verified: true,
    next_url: DEFAULT_FRONTEND_URL + 'admin.html?company_id=' + encodeURIComponent(companyId) + '&id=' + encodeURIComponent(normalizeEmail_((verify.data || {}).email || '') || 'verified_admin') + '&role=Admin&verified=1&token=' + encodeURIComponent(payload.verify_token || '')
  });
}
